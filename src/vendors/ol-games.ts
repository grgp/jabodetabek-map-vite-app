...
ol.control.FrameRate = class olcontrolFrameRate extends ol.control.Control {
  constructor(opt_options) {
    ...
    var element = ol.ext.element.create('DIV', {
      className:
        (options.className || 'ol-framerate') +
...
ol.Sprite = class olSprite extends ol.Feature {
   ...
   */
  update(e) {
    var b = this.image.setState(
      this.currentState,
      (e.frameState.time - this.startState) / this.frate
    );
    if (b && !this.endState) {
      this.dispatchEvent({
        type: 'state',
  ...
  move(e) {
    if (this.moving_) {
      ...
      var dc = [
        this.speed * this.dir[0] * e.dt,
        this.speed * this.dir[1] * e.dt
      ];
      c[0] += dc[0];
      c[1] += dc[1];
      if (this.destination) {
        ...
        if (
          (dx && Math.sign(dx) != Math.sign(this.dir[0])) ||
          ...
          if (this.destination >= this.path.length) {
            ...
            while (true) {
              var ds = ol.coordinate.dist2d(
                this.path[this.destination],
                this.path[this.destination - 1]
              );
              if (ds > dl) break;
              else {
                dl -= ds;
            ...
            this.setCoordinate(this.path[this.destination - 1]);
            this.angle = Math.atan2(
              this.path[this.destination][0] -
                this.path[this.destination - 1][0],
              this.path[this.destination][1] -
                this.path[this.destination - 1][1]
            );
            this.dir = [Math.sin(this.angle), Math.cos(this.angle)];
            this.dispatchEvent({ type: 'change:direction', angle: this.angle });
            // Move remain dist
 ...
 */
ol.featureAnimation.Explode = class olfeatureAnimationExplode extends (
  ol.featureAnimation
) {
  constructor(options) {
    options = options || {};
    ...
    var gradient = (this.gradient = ctx.createRadialGradient(
      0,
      0,
      0,
      0,
    ...
    gradient.addColorStop(
      0.3,
      'rgba(' + [mask(r, 254), mask(g, 239), mask(b, 29), a] + ')'
    );
    gradient.addColorStop(
      0.4,
      'rgba(' + [mask(r, 254), mask(g, 88), mask(b, 29), a] + ')'
    );
    gradient.addColorStop(
      0.6,
      'rgba(' + [mask(r, 239), mask(g, 27), mask(b, 51), a * 0.05] + ')'
    );
    gradient.addColorStop(
      0.88,
      'rgba(' + [mask(r, 153), mask(g, 10), mask(b, 27), a * 0.05] + ')'
    );
    gradient.addColorStop(
      0.92,
      'rgba(' + [mask(r, 254), mask(g, 39), mask(b, 17), a * 0.1] + ')'
    );
    gradient.addColorStop(
      0.98,
      'rgba(' + [mask(r, 254), mask(g, 254), mask(b, 183), a * 0.2] + ')'
    );
    gradient.addColorStop(
      1,
      'rgba(' + [mask(r, 254), mask(g, 39), mask(b, 17), 0] + ')'
    );
...
ol.Backscreen.prototype.setMap = function (map) {
  ...
  if (this.map) {
    ...
    if (!this.map.getViewport().style['z-index'])
      this.map.getViewport().style['z-index'] = 1;
  ...
  dispatch() {
    ...
    for (var i = 0; i < l - 1; i++) {
      for (var j = i + 1; j < l; j++) {
        ...
        if (t) {
          this.sprites[i].dispatchEvent({
            type: 'collide',
            sprite: this.sprites[j],
            hit: t
          });
          this.sprites[j].dispatchEvent({
            type: 'collide',
...
ol.Game = class olGame extends ol.Object {
  constructor(options) {
    ...
    this.collisions = [];
    // Default collision
    this.collision = new ol.Collision({
      game: this,
      resample: options.collisionResample
    });
  }
  /**	Set the game map
   * @param {ol.Map} map
  ...
  timer(msg) {
    if (this._time && msg !== true) {
      console.log(
        '[TIMER] ' + (new Date() - this._time) + ' : ' + (msg || 'timer')
      );
    } else {
...
ol.Offscreen = class olOffscreen extends ol.Object {
  ...
  getPixelValue(pixel) {
    var val = null;
    this.offmap.forEachLayerAtPixel(pixel, function (l, c) {
      if (l && c) {
        val = c;
        return true;
      }
...
ol.graph.Vector = class olgraphVector extends ol.Object {
  constructor(options) {
    ...
    super(options);
    this.edges =
      options.source || new ol.source.Vector({ useSpatialIndex: true });
  ...
  getConnections(node, inout, filter) {
    ...
    var features = this.edges.getFeaturesInExtent([
      p[0] - 1,
      p[1] - 1,
      p[0] + 1,
      p[1] + 1
    ...
    for (var i = 0, c; (c = features[i]); i++)
      if (!filter || filter(c)) {
        if (ol.coordinate.equal(p, c.getGeometry().getFirstCoordinate())) {
          ...
          ol.coordinate.equal(p, c.getGeometry().getLastCoordinate())
        ) {
  ...
  getConnected(edge, inout) {
    ...
    con[0] = this.getConnections(
      edge.getGeometry().getFirstCoordinate(),
      inout,
      filter
    );
    con[1] = this.getConnections(
      edge.getGeometry().getLastCoordinate(),
      inout,
      filter
    );
    return con;
...
ol.media.Media = class olmediaMedia extends ol.Object {
    ...
    if (playPromise !== undefined) {
      playPromise
        ...
        .catch(function () {
          ...
          console.log(
            'Unable to play soud... Let a UI element start before play...'
          );
        });
    }
  }
  ...
  getDurationMS() {
    return (
      Math.floor(this.media.duration / 60) +
      ':' +
      (
        '0' +
        Math.floor(
          this.media.duration - Math.floor(this.media.duration / 60) * 60
        )
      ).substr(-2)
    );
  }
...
ol.source.HexMap = class olsourceHexMap extends ol.source.ImageCanvas {
  ...
  drawHex(extent, res, ratio, size /*, proj*/) {
    ...
    if (pxSize < 8) {
      ...
      cache.width = Math.round(
        w + (dw - pattern.width) * Math.floor(w / pattern.width)
      );
      cache.height = Math.round(
        h + (dh - pattern.height) * Math.floor(h / pattern.height)
      );
      ...
      ctx = this.context;
      ctx.save();
      var o0 = this.grid.hex2offset(
        this.grid.coord2hex([extent[0], extent[1]])
      );
      var o1 = this.grid.hex2offset(
        this.grid.coord2hex([extent[2], extent[3]])
      );
      ctx.scale(ratio, ratio);
      ctx.lineWidth = 1.5;
            ...
            switch (this.coordinates_) {
              ...
              case 'cube': {
                ...
                for (var a = 0; a < 3; a++) {
                  ...
                  ctx.fillText(
                    c[a],
                    p[0] + Math.cos(angle_rad) * pxSize * 0.5,
                    p[1] + Math.sin(angle_rad) * pxSize * 0.5
                  );
                }
                break;
              }
...
ol.style.Sprite = class olstyleSprite extends ol.style.Icon {
  ...
  setState(st, step) {
    ...
    var offset = [
      ((state.start || 0) + (Math.trunc(step) % state.length)) *
        (state.size || this.size),
      (state.line || 0) * (state.size || this.size)
    ];
...
ol.coordinate.projLine = function (c, seg) {
  ...
  return [
    seg[0][0] + s * (seg[1][0] - seg[0][0]),
    seg[0][1] + s * (seg[1][1] - seg[0][1])
  ];
};
